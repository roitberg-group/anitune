...
def _calculate_hash(folds, splits, md5s, shuffle_seed, include_properties):
    ...
    splits_hash = splits_keys + [str(hash(splits[k])) for k in splits_keys] if splits is not None else [str(0)]
    hash_components.extend(splits_hash + [str(folds) if folds is not None else str(0)])
    hash_components.extend(sorted(include_properties) if include_properties is not None else [str(0)])
