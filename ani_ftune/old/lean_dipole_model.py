...
def MLP(atom: str = 'H', dims=None, elements=('H', 'C', 'N', 'O'), classifier_out=None):
    ...
    out = standard(dims_for_atoms[atom], activation=torch.nn.GELU(),
                    bias=False, classifier_out=classifier_out)
...
class DipoleEnergyLean(torch.nn.Module):
    def __init__(self,
        ...
        aev_computer_kwargs = dict() if aev_computer_kwargs is None else aev_computer_kwargs
        if energy_decays[1] is None:
            main_energy_decay = self._calc_decay(energy_decays[0], (3, 4, 4), (4, 8, 10), (4, 9, 10))
        ...
        if charge_decays[1] is None:
            ...
            main_charge_decay = self._calc_decay(charge_decays[0], (2, 3), (5, 6), (6, 8))
        ...
        if embed_charge:
            self._charge_embedding = torch.nn.Embedding(num_embeddings=2 * embed_charge_num, embedding_dim=embed_charge_dim, padding_idx=0)
        ...
        if charge_decays[1] is not None:
            ...
            neighbor_dims = tuple(main_module_dims[-1] // j for j in neighbors_charge_decay)
            final_dims = tuple(neighbor_dims[-1] // j + main_module_dims[-1] // j for j in final_charge_decay)
            charge_module_dicts = [OrderedDict([(e, MLP(e, elements=elements, dims=main_module_dims)) for e in elements])]
            charge_neighbor_module_dicts = [OrderedDict([(e, MLP(e, elements=elements, dims=neighbor_dims)) for e in elements])]
            charge_final_transform = OrderedDict([(e, MLP(e, dims=final_dims, elements=elements, classifier_out=2 if learned_redistribution else 1)) for e in elements])
        else:
            main_module_dims = tuple(in_features // j for j in main_charge_decay)
            charge_module_dicts = [OrderedDict([(e, MLP(e, dims=main_module_dims, elements=elements, classifier_out=2 if learned_redistribution else 1)) for e in elements])]
            charge_neighbor_module_dicts = [OrderedDict()]
            charge_final_transform = OrderedDict()
        self._charge_mpnn = LocalMessagePassing(charge_module_dicts, charge_neighbor_module_dicts, charge_final_transform,
        ...
        if energy_decays[1] is not None:
            ...
            neighbor_dims = tuple(main_module_dims[-1] // j for j in neighbors_energy_decay)
            final_dims = tuple(neighbor_dims[-1] // j + main_module_dims[-1] // j for j in final_energy_decay)

            energy_module_dicts = [OrderedDict([(e, MLP(e, elements=elements, dims=main_module_dims)) for e in elements])]
            energy_neighbor_module_dicts = [OrderedDict([(e, MLP(e, elements=elements, dims=neighbor_dims)) for e in elements])]
            energy_final_transform = OrderedDict([(e, MLP(e, elements=elements, dims=final_dims)) for e in elements])
        else:
            main_module_dims = tuple(in_features // j for j in main_energy_decay)
            energy_module_dicts = [OrderedDict([(e, MLP(e, elements=elements, dims=main_module_dims, classifier_out=1)) for e in elements])]
            energy_neighbor_module_dicts = [OrderedDict()]
            energy_final_transform = OrderedDict()
        self._energy_mpnn = LocalMessagePassing(energy_module_dicts, energy_neighbor_module_dicts, energy_final_transform)

        self.species_converter = SpeciesConverter(elements)
        self.energy_shifter = EnergyShifter(sorted_gsaes(elements, functional=functional, basis_set=basis_set))
    ...
    def forward(self, species_coordinates, total_charges, cell=None, pbc=None):
        ...
        species, charges, precharges = self._charge_mpnn(species, aev, atom_index12, distances, total_charges)
