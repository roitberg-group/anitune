...
if __name__ == "__main__":
    ...
    proc_ds = datasets.ANIDataset('./BondBreaking-reduced-B973c-def2mTZVP_proc.h5', create=True, grouping='by_num_atoms')
    ...
    rep = StandaloneRepulsionCalculator(cutoff=5.1, cutoff_fn=CutoffSmooth(order=2), elements=elements, periodic_table_index=True).to(device)
    disp = StandaloneDispersionD3(cutoff=8.0, cutoff_fn=CutoffSmooth(order=2), elements=elements, periodic_table_index=True, functional='B97-3c').to(device)
    srb = StandaloneEnergySRB(cutoff=5.1, cutoff_fn=CutoffSmooth(order=2), elements=elements, periodic_table_index=True).to(device)
    try:
        max_size = 2500
        for name, j, group in tqdm(ds.chunked_numpy_items(max_size=2500, chem_symbols=False), total=ds.num_conformers / max_size):
            ...
            repulsion_force = -torch.autograd.grad(repulsion_energy.sum(), coordinates)[0]

            coordinates = coordinates.detach().requires_grad_(True)
            dispersion_energy = disp((species, coordinates)).energies
            dispersion_force = -torch.autograd.grad(dispersion_energy.sum(), coordinates)[0]

            coordinates = coordinates.detach().requires_grad_(True)
            srb_energy = srb((species, coordinates)).energies
            srb_force = -torch.autograd.grad(srb_energy.sum(), coordinates)[0]

            extra_keys['D3-two-body.energy-corrections'].append(dispersion_energy.detach().cpu())
            extra_keys['D3-two-body.force-corrections'].append(dispersion_force.detach().cpu())
            extra_keys['xTB-repulsion.energy-corrections'].append(repulsion_energy.detach().cpu())
            extra_keys['xTB-repulsion.force-corrections'].append(repulsion_force.detach().cpu())
            ...
            extra_keys['processed_energies'] = group['energies'] - extra_keys['D3-two-body.energy-corrections'] - extra_keys['xTB-repulsion.energy-corrections']
            extra_keys['processed_forces'] = group['forces'] - extra_keys['D3-two-body.force-corrections'] - extra_keys['xTB-repulsion.force-corrections']
