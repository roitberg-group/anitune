...
def dipole_pass(atom: str = "H", dims=None):
    ...
    dims_for_atoms = {"H": dim_tuple, "C": dim_tuple, "N": dim_tuple, "O": dim_tuple}
    out = standard(
        dims_for_atoms[atom],
...
def autodipole_simple_transform_pass(
    atom: str = "H", dims=None, elements=("H", "C", "N", "O")
):
    dim_tuple = (96, 48) if dims is None else dims
...
def dipole_simple_transform(atom: str = "H", dims=None):
    ...
    dims_for_atoms = {"H": dim_tuple, "C": dim_tuple, "N": dim_tuple, "O": dim_tuple}
    out = standard(
        dims_for_atoms[atom], activation=torch.nn.GELU(), bias=False, classifier_out=1
    )
    out.out_size = dims_for_atoms["H"][-1]
    return out
...
def dipole_nonlocal_transform(atom: str = "H", dims=None):
    ...
    dims_for_atoms = {"H": dim_tuple, "C": dim_tuple, "N": dim_tuple, "O": dim_tuple}
    out = standard(
        dims_for_atoms[atom], activation=torch.nn.GELU(), bias=False, classifier_out=2
    )
    out.out_size = dims_for_atoms["H"][-1]
    return out
    ...
    def __init__(
        ...
        if embed_charge:
            ...
            self._charge_embedding = torch.nn.Embedding(
                num_embeddings=14, embedding_dim=module_out_sizes[0], padding_idx=0
            )
            ...
            if embed_start_dim is not None:
                warnings.warn(
                    "embed start dim is deprecated, please use charge_embed_dim"
                )
                charge_embed_dim = embed_start_dim
            self._charge_embedding = torch.nn.Embedding(
                num_embeddings=14, embedding_dim=charge_embed_dim, padding_idx=0
            )
    ...
    def forward(
        ...
        for j, (module_pass, neighbor_pass) in enumerate(
            ...
            if self.embed_charge and j == 0:
                assert not (
                    total_charges < -6
                ).any(), "less than -6 charge not supported"
                embedding = total_charges + 7
                embedding = embedding.unsqueeze(1).masked_fill(species == -1, 0).long()
                embedding = self._charge_embedding(embedding)
            ...
            neighbor_features = neighbor_features[atom_index12.flipud()] * decay.view(
                1, -1, 1
            )
            neighbor_merged_features.index_add_(
                0, atom_index12.view(-1), neighbor_features.view(-1, neighbor_out_size)
            )
        ...
        if total_charges is not None:
            ...
            if self.nonlocal_charge_redistribution:
                distribution_factors = (
                    torch.exp(-(distribution_factors**2)) + 1e-12
                )  # epsilon for regulatization of division
                if self.regularize:
...
class SimpleLocalDipoleMP(torch.nn.Module):
    def __init__(
        ...
        final_dims=None,
        embed_charge=False,
        embed_at_start=False,
        embed_start_dim=20,
        nonlocal_charge_redistribution=False,
        charge_embed_dim: int = 14,
    ):
        super().__init__()
        ...
        neighbor_module_dicts = [
            OrderedDict([(e, dipole_neighbor_pass(e, neighbor_dims)) for e in elements])
        ]
        if nonlocal_charge_redistribution:
            simple_transform = OrderedDict(
                [(e, dipole_nonlocal_transform(e, final_dims)) for e in elements]
            )
        else:
            simple_transform = OrderedDict(
                [(e, dipole_simple_transform(e, final_dims)) for e in elements]
            )

        self._mpnn = LocalMessagePassing(
            module_dicts,
            neighbor_module_dicts,
            simple_transform,
            lembed_charge=embed_charge,
    ...
    def forward(self, species_coordinates, total_charges, cell=None, pbc=None):
        ...
        atom_index12, _, diff_vectors, distances = self._aev_computer.neighborlist(
            species, coordinates, cell, pbc
        )
        aev = self._aev_computer._compute_aev(
            species, atom_index12, diff_vectors, distances
        )
        species, charges, precharges = self._mpnn(
...
class DipolesAndEnergies1x(torch.nn.Module):
    def __init__(
        ...
        if atomic_maker is not None:
            energies_model = model_class(
                **energy_model_kwargs, atomic_maker=atomic_maker
            )
        else:
        ...
        if coulomb_terms:
            coulomb_eta = (
                torch.tensor([3.5])
                if coulomb_eta is None
                else torch.tensor(coulomb_eta)
            )
        ...
        if embed_and_couple_start:
            ...
            if embed_and_couple_dim_start:
                warnings.warn(
                    "embed_and_couple_dim_start is deprecated please use charge_embed_dim"
                )
