...
class Runner:
    def __init__(
        ...
        self._transform = (
            ...
            else transforms.Identity().to(device)
        )
    ...
    def inner_loop(
        self, batch, metrics: Dict[str, Tensor], train: bool = False
    ) -> Tuple[Tensor, int]:
    ...
    def _add_kcalpermol_metrics(self, metrics: ScalarMetrics):
        for k in metrics.copy().keys():
            if k.endswith("_hartree"):
                metrics[k.replace("_hartree", "_kcalpermol")] = units.hartree2kcalmol(
                    metrics[k]
                )
            elif k.endswith("_hartree_per_angstrom"):
                metrics[
                    k.replace("_hartree_per_angstrom", "_kcalpermol_per_angstrom")
                ] = units.hartree2kcalmol(metrics[k])
            elif k.endswith("_eA"):
    ...
    def _print_metrics(
        self, split: str, metrics: ScalarMetrics, epoch: Optional[int] = None
    ):
        print(
            f"epoch {epoch}, {split} metrics:"
            if epoch is not None
            else f"{split} metrics:"
        )
...
class MultiRunner(Runner):
    _DEFAULT_LOSSES = {
        ...
        "diss": 1e-4,
        "charge": 1.0,
        "force_smoothness": 1e-4,
        "charge_smoothness": 1e-4,
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._energies_key = (
            "energies" if not processed_energies else "processed_energies"
        )
    ...
    def inner_loop(
        ...
        with torch.enable_grad():
            ...
            if "dipole" in self._losses.keys() or "charge" in self._losses.keys():
                ...
                if "charge" in self._losses.keys():
                    _, predicted_energies, predicted_charges, predicted_precharges = (
                        self._model((species, coordinates), total_charges=total_charges)
                    )
                else:
                    _, predicted_energies, predicted_charges = self._model(
                        (species, coordinates), total_charges=total_charges
                    )
                if "charge_smoothness" in self._losses.keys():
                    charge_derivatives = torch.autograd.grad(
                        predicted_charges.sum(),
                        coordinates,
                        create_graph=True,
                        retain_graph=True,
                    )[0]
                    batch_loss["charge_smoothness"] = charge_derivatives.pow(2).sum(
                        dim=(1, 2)
                    )
            else:
                predicted_energies = self._model((species, coordinates)).energies
            ...
            if "energy" in self._losses.keys():
                squared_energy_error = self._squared_error(
        ...
        if "diss" in self._losses.keys():
            ...
            batch_loss["diss"] = (
                self._relu(
                    shifter.sae(converter((species, coordinates))[0])
                    - predicted_energies
                )
                ** 2
            )
        ...
        if "charge" in self._losses.keys():
            squared_charge_error = self._squared_error(
                total_charges, predicted_precharges.sum(-1)
            )
            batch_loss["charge"] = squared_charge_error

        if "dipole" in self._losses.keys():
            _, displaced_coordinates = geometry.displace_to_com_frame(
                (species, coordinates)
            )
            predicted_dipoles = (
                displaced_coordinates * predicted_charges.unsqueeze(-1)
            ).sum(-2)
            squared_dipole_error = self._squared_error(
                target_dipoles, predicted_dipoles
            ).sum(-1)
            batch_loss["dipole"] = squared_dipole_error / 3

        if "energy" in self._losses.keys():
            metrics["energy_rmse_hartree"] += squared_energy_error.detach().sum().item()
            metrics["energy_mae_hartree"] += (
                torch.abs(predicted_energies.detach() - target_energies.detach())
                .sum()
                .item()
            )
        if "force" in self._losses.keys():
            metrics["force_rmse_hartree_per_angstrom"] += (
                batch_loss["force"].detach().sum().item()
            )
            metrics["force_mae_hartree_per_angstrom"] += (
                (torch.sqrt(squared_force_error.detach()) / (3 * num_atoms))
                .sum()
                .item()
            )
        if "dipole" in self._losses.keys():
            metrics["dipole_rmse_eA"] += batch_loss["dipole"].detach().sum().item()
            metrics["dipole_mae_eA"] += (
                (torch.sqrt(squared_dipole_error.detach()) / 3).sum().item()
            )
        if "charge" in self._losses.keys():
            metrics["charge_rmse_e"] += batch_loss["charge"].detach().sum().item()
            metrics["charge_mae_e"] += (
                (torch.sqrt(squared_charge_error.detach())).sum().item()
            )
    ...
    def set_extra_metrics(self):
        ...
        if "force" in self._losses.keys():
            metrics.update(
                {
                    "force_rmse_hartree_per_angstrom": 0.0,
                    "force_mae_hartree_per_angstrom": 0.0,
                }
            )
        if "dipole" in self._losses.keys():
