...
def extract_class_parameters(config, module, key_name: str, extra_args: Tuple[Any, ...] = tuple(), extra_kwargs: Dict[str, Any] = None):
...
def main():
    ...
    parser.add_argument('--config', required=True, help="Configuration file for the run")
    ...
    if model_num == -1:
        ...
        SPLITS = data_splits if data_splits is not None else {'training': 0.8, 'validation': 0.2}
    else:
        SPLITS = None
    ...
    training_set, validation_set = core.prepare_learning_sets(config["data"], SPLITS, FOLDS,
                                                                  DATASET_ROOT_DIR, TRAINING_SPLIT, VALIDATION_SPLIT)
    ...
    model = extract_class_parameters(config, builders, "model", extra_kwargs=extra_kwargs)
    ...
    # GSAEs
    if use_gsaes:
        gsaes = sorted_gsaes(model.get_chemical_symbols(), functional=config["data"]["functional"], basis_set=config["data"]["basis_set"])
    ...
    optimizer = extract_class_parameters(config, torch.optim, "optimizer", extra_args=(opt_params,))
    scheduler = extract_class_parameters(config, lr_scheduler, "scheduler", extra_args=(optimizer,))
    runner_kwargs = {"model": model, "optimizer": optimizer, "device": DEVICE}
    if "uw_loss" in config.keys():
        runner_kwargs.update({"uw_loss": uw_loss})
    runner = extract_class_parameters(config, runners, "runner", extra_kwargs=runner_kwargs)
    ...
    core.execute_training(persistent_objects, scheduler, runner, optimizer,
                              training_set, validation_set, run_output_path, config, debug=DEBUG)
