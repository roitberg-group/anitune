...
class Logger:
    def __init__(self,
        ...
        if log_wandb:
            assert WANDB_AVAIL
            if not debug:
                wandb.init(project='qrnn-nacho', name=path.name, entity='roitberg-group')
    ...
    def _dump_csv_metrics(self, step: int, path: PathLike, metrics: ScalarMetrics, first_step: int):
...
def prepare_learning_sets(data_config: Dict[str, Any], splits: Optional[int],
                          folds: Optional[int], root_dataset_path: Path,
                          training_split: str, validation_split: str):
    ...
    digest = _calculate_hash(folds, splits, sorted(ds.store_md5s), shuffle_seed, include_properties)

    if splits is not None:
        batched_dataset_path = Path(ds_path.as_posix() + f'-batched-{digest}').resolve()
    else:
        batched_dataset_path = Path(ds_path.as_posix() + f'-ensemble-batched-{digest}').resolve()
    ...
    training_set = torch.utils.data.DataLoader(torchani.datasets.ANIBatchedDataset(batched_dataset_path,
    ...
    validation_set = torch.utils.data.DataLoader(torchani.datasets.ANIBatchedDataset(batched_dataset_path,
...
def execute_training(persistent_objects: Dict[str, Stateful], scheduler: LRScheduler, runner: Runner, optimizer: Optimizer,
                     training_set, validation_set, run_output_path: Path, config: Dict[str, Any], debug=False):
