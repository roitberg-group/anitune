...
def light_ions_atomics(atom: str = "H"):
    ...
    return standard(dims_for_atoms[atom], activation=torch.nn.GELU(), bias=False)
...
class UncertaintyWeightedLoss(torch.nn.Module):
    def __init__(self, keys: Iterable[str]):
        ...
        self.params = ParameterDict(
            {k: Parameter(torch.tensor(0.0, dtype=torch.float)) for k in keys}
        )
